...
def clean_article_content(article_content):
    ...
    # 여러 줄로 나누어진 패턴 제거 및 특수 문자 제거
    return clean_text(article_content.strip())
...
def get_article_content(url):
    ...
    for attempt in range(retry_count):
        try:
            ...
            print(f"Timeout error on attempt {attempt + 1}: {url}")
        except requests.exceptions.RequestException as e:
            print(f"Error crawling {url}: {e}")
    print(f"Failed to crawl after {retry_count} attempts: {url}")
    return None
...
def collect_relevant_stock_news_kr():
    ...
    # JSON 파일 초기화
    if not os.path.exists('news_data_clean.json'):
        with open('news_data_clean.json', 'w', encoding='utf-8') as f:
            json.dump([], f)  # 빈 JSON 리스트 생성
        ...
        for keyword in stock_prediction_keywords_kr:
            ...
            while collected_articles_count < total_articles_limit:
                ...
                for item in news_result.get('items', []):
                    link = item['link']
                    title = clean_text(item['title'])  # 제목에서 특수 문자 제거
                    description = clean_text(item['description'])  # 설명에서도 특수 문자 제거
                    ...
                    if not hashlib.md5(link.encode()).hexdigest() in articles:
                        ...
                        # 기사 데이터 저장
                        article_data = {
                            'title': title,  # 정제된 제목 저장
                            'link': link,
                            'description': description,  # 정제된 설명 저장
                            'content': article_content,  # 정제된 기사 본문 저장
                            'pub_date': pub_date,
                            'stock_keywords': expanded_keywords  # 주식 관련 확장 키워드 저장
                        }

                        # 기존 데이터 불러오기 및 추가
                        with open('news_data_clean.json', 'r+', encoding='utf-8') as f:
                            data = json.load(f)
